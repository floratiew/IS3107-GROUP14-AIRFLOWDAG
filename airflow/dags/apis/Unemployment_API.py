...
def get_unemployment_data():
    ...
    if response.status_code == 200: #check if HTTP request to API is successful
        ...
        df_expanded = df_expanded[df_expanded['Year'] >= 1995] #filter out years before 1995
...
def forecast_unemployment_with_fallback(df):
    ...
    target_date = datetime(year=target_year, month=12, day=1)
    ...
    if len(group) >= 12:
        ...
        m.add_seasonality(name='quarterly', period=3, fourier_order=3)
        m.fit(model_df)
